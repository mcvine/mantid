...
void Histogram::checkSize(const PointVariances &points) const {
  size_t target = m_x->size();
  // 0 edges -> 0 points, otherwise points are 1 less than edges.
  if (xMode() == XMode::BinEdges && target > 0)
    target--;
  if (target != points.size())
    throw std::logic_error("Histogram: size mismatch of PointVariances\n");
}

void Histogram::checkSize(const PointStandardDeviations &points) const {
  size_t target = m_x->size();
  // 0 edges -> 0 points, otherwise points are 1 less than edges.
  if (xMode() == XMode::BinEdges && target > 0)
    target--;
  if (target != points.size())
    throw std::logic_error(
        "Histogram: size mismatch of PointStandardDeviations\n");
}
